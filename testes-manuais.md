...
```bash
curl -i http://localhost:9002/pagamentos-ms/pagamentos
```
...
```bash
curl -i -X POST http://localhost:9002/auth-ms/auth/registro \
...
```bash
curl -i -X POST http://localhost:9002/auth-ms/auth/login \
  -H "Content-Type: application/json" \
...
```bash
curl -i http://localhost:9002/pedidos-ms/pedidos \
  -H "Authorization: Bearer <SEU_TOKEN>"
```
...
```bash
curl -i "http://localhost:9002/auth-ms/auth/validar?token=<SEU_TOKEN>"
```
